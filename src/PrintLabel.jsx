...
function LabelContent({ box, pageIndex, totalPages, isPreview }) {
  ...
  const fs = (pt) => isPreview ? `${Math.round(pt * 1.1)}px` : `${pt}pt`;

  return (
    <div style={{
      width: '100%',
      height: '100%',
      padding: isPreview ? '6px' : '0.2cm',
      ...
      <div style={{
        ...
        color: '#fff',
        textAlign: 'center',
        padding: isPreview ? '2px 3px' : '2pt 3pt',
        fontSize: fs(6),
        fontWeight: 900,
        lineHeight: 1.2,
        flexShrink: 0,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexShrink: 0 }}>
        <div>
          <div style={{ fontWeight: 900, fontSize: fs(9) }}>PT DUA PULUH TIGA</div>
          <div style={{ fontSize: fs(5.5), color: '#444' }}>Jl. Kopo Bihbul Raya No 68, Bandung</div>
          <div style={{ fontSize: fs(5), marginTop: '2px' }}>HUID: <b>{box.huid}</b></div>
        </div>
        <div style={{ flexShrink: 0 }}>
          <QRCode value={box.huid || '-'} size={isPreview ? 38 : 50} level="H" />
        </div>
      </div>
      ...
      {/* PICKLIST + TOKO */}
      <div style={{ flexShrink: 0 }}>
        <div style={{ fontWeight: 900, fontSize: fs(7.5) }}>{box.picklist_number}</div>
        <div style={{ fontWeight: 900, fontSize: fs(7.5) }}>{box.nama_toko || '-'}</div>
        <div style={{ fontSize: fs(5.5), color: '#555' }}>{box.alamat_toko || '-'}</div>
      </div>
      ...
      <div style={{
        ...
        fontWeight: 900,
        background: '#f0f0f0',
        padding: isPreview ? '1px 2px' : '1pt 2pt',
        flexShrink: 0,
      }}>
      ...
      <table style={{
        ...
        <thead>
          <tr style={{ background: '#000', color: '#fff' }}>
            <th style={{ padding: isPreview ? '1px 2px' : '1pt 2pt', textAlign: 'left', width: '28%' }}>Artikel</th>
            <th style={{ padding: isPreview ? '1px 2px' : '1pt 2pt', textAlign: 'left' }}>Description</th>
            <th style={{ padding: isPreview ? '1px 2px' : '1pt 2pt', textAlign: 'center', width: '12%' }}>Qty</th>
          </tr>
        </thead>
        <tbody>
          {pItems.map((it, k) => (
            <tr key={k} style={{ borderBottom: '0.5px solid #ccc', background: k % 2 === 0 ? '#fff' : '#f9f9f9' }}>
              <td style={{ padding: isPreview ? '0.5px 2px' : '0.5pt 2pt', fontWeight: 700, overflow: 'hidden' }}>{it?.sku}</td>
              <td style={{ padding: isPreview ? '0.5px 2px' : '0.5pt 2pt', color: '#333', overflow: 'hidden', maxWidth: 0, textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{it?.nama_item || it?.name || '-'}</td>
              <td style={{ padding: isPreview ? '0.5px 2px' : '0.5pt 2pt', textAlign: 'center', fontWeight: 900 }}>{it?.qty}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div style={{ textAlign: 'center', flexShrink: 0, marginTop: 'auto' }}>
        <div style={{ fontSize: fs(5), fontWeight: 700 }}>NO SJ: {box.no_sj || box.picklist_number}</div>
        <Barcode
          value={box.no_sj || box.picklist_number || 'NOSJ'}
          width={isPreview ? 0.9 : 1.6}
          height={isPreview ? 22 : 35}
          fontSize={0}
          margin={0}
        />
      </div>
...
/* ================= PRINT LABEL PANEL ================= */
export function PrintLabelPanel({
  data,
  selectedPcb, setSelectedPcb,
  selectedBoxHuid, setSelectedBoxHuid,
  boxOptions, fetchBoxByPcb,
  loading,
}) {
  ...
  const handlePrint = () => {
    ...
    const box = currentPrintData;
    if (!box) return;

    const pages = Math.max(1, Math.ceil((box.item_details || []).length / 12));
    for (let i = 0; i < pages; i++) {
      const items = (box.item_details || []).slice(i * 12, (i + 1) * 12);
      const rows = items.map((it, k) =>
        `<tr style="border-bottom:0.5px solid #ccc;background:${k % 2 === 0 ? '#fff' : '#f9f9f9'}">
          <td style="padding:0.5pt 2pt;font-weight:700;font-size:5.5pt">${it?.sku || ''}</td>
          <td style="padding:0.5pt 2pt;font-size:5.5pt;overflow:hidden">${it?.nama_item || it?.name || '-'}</td>
          <td style="padding:0.5pt 2pt;text-align:center;font-weight:900;font-size:5.5pt">${it?.qty || ''}</td>
        </tr>`
      ).join('');
      ...
      page.innerHTML = `
        <div style="background:#c00;color:#fff;text-align:center;padding:2pt 3pt;font-size:6pt;font-weight:900;flex-shrink:0;">
          ⚠ WAJIB VIDEO UNBOXING — KOMPLAIN TANPA VIDEO TIDAK DILAYANI
        </div>
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-shrink:0;margin-top:2pt;">
          <div>
            <div style="font-weight:900;font-size:9pt;">PT DUA PULUH TIGA</div>
            <div style="font-size:5.5pt;color:#444;">Jl. Kopo Bihbul Raya No 68, Bandung</div>
            <div style="font-size:5pt;margin-top:1pt;">HUID: <b>${box.huid}</b></div>
            <div style="font-weight:900;font-size:7.5pt;margin-top:2pt;">${box.picklist_number}</div>
            <div style="font-weight:900;font-size:7.5pt;">${box.nama_toko || '-'}</div>
            <div style="font-size:5.5pt;color:#555;">${box.alamat_toko || '-'}</div>
          </div>
        </div>
        <div style="border-top:1px solid #000;margin:2pt 0;flex-shrink:0;"></div>
        <div style="display:flex;justify-content:space-between;font-size:5.5pt;font-weight:900;background:#f0f0f0;padding:1pt 2pt;flex-shrink:0;">
          <span>BOX CONTENT</span><span>BOX: ${box.container_number || '1'} (${i + 1}/${pages})</span>
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:5.5pt;flex:1;overflow:hidden;">
          <thead>
            <tr style="background:#000;color:#fff;">
              <th style="padding:1pt 2pt;text-align:left;width:28%;">Artikel</th>
              <th style="padding:1pt 2pt;text-align:left;">Description</th>
              <th style="padding:1pt 2pt;text-align:center;width:12%;">Qty</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
        <div style="border-top:1px solid #000;margin:1pt 0;flex-shrink:0;"></div>
        <div style="display:grid;grid-template-columns:1fr 1fr;font-size:5pt;flex-shrink:0;">
          <div>Packer: <b>${box.packer_name || '-'}</b></div>
          <div style="text-align:right;">Total: <b>${box.total_pcs_box || 0} PCS</b></div>
          <div>Tgl: <b>${box.tanggal_packing?.substring(0, 10) || '-'}</b></div>
          <div style="text-align:right;">Berat: <b>${box.weight_kg || '0'} KG</b></div>
        </div>
        <div style="text-align:center;flex-shrink:0;margin-top:auto;">
          <div style="font-size:5pt;font-weight:700;">NO SJ: ${box.no_sj || box.picklist_number}</div>
        </div>
      `;
    ...
    window.print();
    setTimeout(() => { if (root) root.innerHTML = ''; }, 1500);
  };
  ...
  return (
    <div style={{ display: 'flex', gap: '24px', height: 'calc(100vh - 180px)', fontFamily: 'Lexend, sans-serif' }}>
      ...
      <div style={{ width: '260px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        {currentPrintData && (
          <button onClick={handlePrint} style={printBtn}>
            🖨 PRINT LABEL ({totalPages} halaman)
          </button>
        )}
      </div>
      ...
      <div style={{
        ...
        flexDirection: 'column',
        alignItems: 'center',
        justifyContent: currentPrintData ? 'flex-start' : 'center',
        background: '#f5f5f5',
        borderRadius: '12px',
        padding: '24px',
        overflow: 'auto',
              ...
              <div style={{ display: 'flex', gap: '6px', marginTop: '12px' }}>
                {Array.from({ length: totalPages }).map((_, i) => (
                  <div key={i} onClick={() => setPreviewPage(i)} style={{
                    ...
                    background: i === previewPage ? '#000' : '#ddd',
                    cursor: 'pointer',
                  }} />
                ))}
              </div>
