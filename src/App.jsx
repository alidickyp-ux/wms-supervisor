...
function App() {
  ...
  const handleAddLoc = async () => {
    if (!newLoc.id || !newLoc.unique) return showToast("ID & Group wajib isi", "error");
    try {
  ...
  return (
    <div style={mainLayout}>
      ...
      <div style={contentArea(isMobile)}>
        <header style={headerStyle}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {!isMobile && activeMenu === 'Master Lokasi' && masterTab === 'database' && (
              <>
                <button onClick={handleBulkDelete} disabled={selectedRows.length === 0} style={{...btnWhite, color:'red'}}><Trash2 size={12}/> DELETE ({selectedRows.length})</button>
        ...
        {showAddForm && (
          <div style={popupOverlay}>
            <div style={{...popupContent, textAlign:'left', padding:30}}>
              <div style={{display:'flex', justifyContent:'space-between', marginBottom:20}}><h3 style={{fontWeight:900}}>ADD NEW</h3><button onClick={()=>setShowAddForm(false)} style={{border:'none', background:'none'}}><X size={20}/></button></div>
              <label style={labelStyle}>LOCATION ID (BARCODE)</label>
              <input style={mInput} value={newLoc.id} onChange={e=>setNewLoc({...newLoc, id: e.target.value})} placeholder="e.g. 00104014" />
        ...
        {(masterTab === 'database' || activeMenu !== 'Master Lokasi') && !isMobile && (
            ...
            <table style={tableStyle}>
              <thead style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#fff' }}>
                <tr style={{ background: '#fafafa' }}>
                  ...
                  <th style={thStyle}>LOKASI ID</th>
                  {activeMenu === 'Master Lokasi' ? <><th style={thStyle}>ZONE</th><th style={thStyle}>AISLE</th><th style={thStyle}>GROUP</th><th style={thStyle}>STATUS</th></> : <><th style={thStyle}>ARTIKEL</th><th style={thStyle}>QTY</th><th style={thStyle}>DIFF</th><th style={thStyle}>DESC</th></>}
                </tr>
              </thead>
              <tbody>
                {filteredData.map((row, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    {activeMenu === 'Master Lokasi' ? <><td style={tdStyle}>{row.zone || '-'}</td><td style={tdStyle}>{row.aisle || '-'}</td><td style={tdStyle}>{row.unique_id}</td><td style={{...tdStyle, fontWeight:800, color: row.assign === 'open' ? 'green' : 'red'}}>{row.assign?.toUpperCase()}</td></> : <><td style={tdStyle}>{row.artikel}</td><td style={tdStyle}>{row.qty_snap || row.qty_1st}</td><td style={tdStyle}>{row.final_status === 'MATCH' ? 0 : (Number(row.qty_2nd || row.qty_1st || 0) - Number(row.qty_snap || 0))}</td><td style={tdDescSmall}>{getDesc(row)}</td></>}
                  </tr>
                ))}
              </tbody>
            </table>
