...
function App() {
  ...
  const fetchBoxByPcb = async (pcb) => {
    ...
    try {
      ...
      showToast("Gagal tarik box detail", "error"); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (res.data?.status === 'success') { 
        ...
        showToast("User/Pass Salah", "error");
      }
    } catch (e) { 
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (const id of selectedRows) { 
        await axios.post(`${API_BASE}?action=delete_location`, { unique_id: id }); 
      }
  ...
  const filteredData = (data || []).filter(item => {
    ...
    return String(item.location_id || item.picklist_number || '').includes(s) || String(item.artikel || '').includes(s);
  });
      ...
      <div style={contentArea(isMobile)} className="no-print">
            ...
            {!isMobile && (
              <>
                {activeMenu === 'Master Lokasi' && masterTab === 'database' && (
                  <>
                    <button onClick={handleBulkDelete} style={{...btnWhite, color:'red'}}><Trash2 size={12}/> DELETE</button>
        ...
        {isMobile && !showMobileHome && (
          <div style={{marginBottom: 15}}>
             <input placeholder={`Search ${activeMenu}...`} style={mInput} value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} />
             {['1st Count', '2nt Count'].includes(activeMenu) && (
               <div style={formWrapper}>
                  <input ref={inputLocRef} placeholder="SCAN LOKASI" style={mInput} value={mobLoc} onChange={e=>setMobLoc(e.target.value.toUpperCase())} />
                  <input ref={inputArtRef} placeholder="SCAN ARTIKEL" style={mInput} value={mobArt} onChange={e=>setMobArt(e.target.value.toUpperCase())} />
                  <input ref={inputQtyRef} type="number" placeholder="QTY" style={qtyInput} value={mobQty} onChange={e=>setMobQty(e.target.value)} />
                  <button onClick={handleSaveInput} style={btnBlack} disabled={loading}>{loading ? 'SAVING...' : 'SAVE DATA'}</button>
               </div>
             )}
          </div>
        )}
           ...
           <div style={{display:'flex', gap:20, flexDirection: isMobile ? 'column':'row'}}>
              ...
              <div style={{flex:1.5, background:'#f5f5f5', padding:20, borderRadius:8, display:'flex', justifyContent:'center', minHeight: '520px'}}>
                 <div style={{transform:'scale(0.7)', background:'#fff', width:'10cm', height:'10cm', border:'1px solid #ddd'}}>
                    <RenderLabel box={currentBoxData} />
                 </div>
              </div>
           </div>
        ) : (
                ...
                <thead>
                   <tr style={{background:'#fafafa'}}>
                      ...
                      {activeMenu === 'Reconciliation' ? (
                        ...
                        <><th style={thStyle}>LOKASI</th><th style={thStyle}>ARTIKEL</th><th style={thStyle}>QTY</th><th style={thStyle}>TIME / DESC</th></>
                      )}
                   </tr>
                </thead>
                <tbody>
                   {filteredData.map((row, i)=>(
                     <tr key={i} style={{borderBottom:'1px solid #eee'}}>
                        ...
                        <td style={tdStyle}>{row.location_id || row.unique_id || row.container_number}</td>
                        <td style={tdStyle}>{row.artikel || row.product_id}</td>
                        <td style={tdStyle}>{row.qty_snap || row.qty_1st || row.qty_packed || row.qty_actual}</td>
                        <td style={tdStyle}>{activeMenu.includes('Picking') || activeMenu.includes('Packing') ? formatWIB(row.scanned_at) : getDesc(row)}</td>
                     </tr>
                   ))}
                </tbody>
             </table>
      ...
      {showAddForm && (
        <div style={popupOverlay}>
          <div style={{...popupContent, textAlign:'left', padding:30}}>
            ...
            <label style={labelStyle}>ZONE</label><input style={mInput} value={newLoc.zone} onChange={e=>setNewLoc({...newLoc, zone: e.target.value.toUpperCase()})} />
            <label style={labelStyle}>AISLE</label><input style={mInput} type="number" value={newLoc.aisle} onChange={e=>setNewLoc({...newLoc, aisle: e.target.value})} />
      ...
      {pages.map((pItems, idx) => (
        <div key={idx} className="l-page">
          ...
          <div className="l-header">
             ...
             <div style={{textAlign:'right'}}><div className="l-huid">HUID: <b>{box.huid}</b></div><QRCode value={box.huid || ''} size={42}/><div style={{fontSize:'6pt'}}>{box.huid}</div></div>
          </div>
          <div className="l-line" />
          <div className="l-grid"><div>{box.picklist_number}</div><div>{box.nama_toko}</div><div style={{fontSize:'7pt'}}>{box.alamat_toko || '-'}</div></div>
          <div className="l-line" />
          <div style={{fontWeight:900, fontSize:'8pt', padding:'2px'}}>BOX CONTENT ({idx+1}/{pages.length})</div>
...
const btnBlack = { width: '100%', background: '#000', color: '#fff', padding: '14px', border: 'none', borderRadius: '8px', fontWeight: '800', cursor: 'pointer' };
