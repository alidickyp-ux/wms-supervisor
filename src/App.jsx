...
import {
  ...
  CheckCircle2, Loader2, Check, Plus, Database as DbIcon, LayoutGrid
} from 'lucide-react';
...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      axios.get(`${API_BASE}?action=get_data&target=snapshot_list`).then(rs => {
        setSnapData(Array.isArray(rs.data.data) ? rs.data.data : []);
      });
      
      axios.get(`${API_BASE}?action=get_data&target=recon`).then(rr => {
        window.reconCacheData = Array.isArray(rr.data.data) ? rr.data.data : [];
      });
    } catch (e) { setData([]); }
  ...
  const handleAddLoc = async () => {
    ...
    try {
      await axios.post(`${API_BASE}?action=add_location`, { location_id: newLocName.toUpperCase() });
      showToast("Location Added!");
      setNewLocName(''); fetchData();
    } catch (e) { showToast("Add Failed", "error"); }
  };

  const handleDeleteLoc = async (uid) => {
    if (!window.confirm(`Delete ${uid}?`)) return;
    try {
    ...
    try {
      ...
      if (activeMenu === '1st Count' && locInfo) {
        const remain = locInfo.filter(item => {
          const isCurr = String(item.artikel).toUpperCase() === currentArt;
          const isDone = data.some(d => String(d.location_id).toUpperCase() === currentLoc && String(d.artikel).toUpperCase() === String(item.artikel).toUpperCase());
          return !isCurr && !isDone;
        });
        if (remain.length === 0) isFinished = true;
      } else if (activeMenu === '2nt Count') {
        const remain2nd = (window.reconCacheData || []).filter(d => 
          String(d.location_id).toUpperCase() === currentLoc && 
          d.final_status === 'NEED 2ND COUNT' && 
          String(d.artikel).toUpperCase() !== currentArt
        );
        if (remain2nd.length === 0) isFinished = true;
      ...
      <div style={contentArea(isMobile)}>
              ...
              <>
                {activeMenu === 'Master Lokasi' && masterTab === 'database' && (
                  <div style={{display:'flex', gap:5}}>
                    <input placeholder="New Loc..." style={{...mInput, marginBottom:0, width:100, padding:'6px'}} value={newLocName} onChange={e=>setNewLocName(e.target.value)} />
                    <button onClick={handleAddLoc} style={{...btnWhite, background:'#000', color:'#fff'}}><Plus size={12}/> ADD</button>
                  </div>
                )}
                {activeMenu === 'Snapshoot' && (
                  ...
                  <button onClick={() => { if(window.confirm("Clear?")) axios.post(`${API_BASE}?action=clear_snap`).then(()=>fetchData()); }} style={{...btnWhite, color:'red'}}><Trash2 size={12}/></button></>
                )}
                {['1st Count', '2nt Count'].includes(activeMenu) && (
                  ...
                  <button onClick={() => { if(window.confirm("Clear?")) axios.post(`${API_BASE}?action=clear_${activeMenu === '1st Count' ? 'first' : 'second'}`).then(()=>fetchData()); }} style={{...btnWhite, color:'red'}}><Trash2 size={12}/></button></>
                )}
              ...
              <thead style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#fff' }}>
                <tr style={{ background: '#fafafa' }}>
                  {activeMenu === 'Master Lokasi' ? (
                    <><th style={thStyle}>LOKASI</th><th style={thStyle}>ZONE</th><th style={thStyle}>AISLE</th><th style={thStyle}>UNIQUE ID</th><th style={thStyle}>ASSIGN</th><th style={thStyle}>ACTION</th></>
                  ) : (
                  ...
                  : activeMenu === 'Master Lokasi' ? null : <><th style={thStyle}>DESC</th><th style={thStyle}>QTY</th><th style={thStyle}>OP</th></>}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      {activeMenu === 'Master Lokasi' ? (
                        <>
                          ...
                          <td style={{...tdStyle, fontWeight:800, color: row.assign === 'open' ? 'green' : 'red'}}>{row.assign?.toUpperCase()}</td>
                          <td style={tdStyle}><button onClick={()=>handleDeleteLoc(row.unique_id)} style={{color:'red', border:'none', background:'none', cursor:'pointer'}}><Trash2 size={14}/></button></td>
                        </>
                      ...
                      ) : activeMenu === 'Master Lokasi' ? null : <><td style={tdDescSmall}>{getDesc(row)}</td><td style={tdStyle}>{row.qty_1st || row.qty_2nd}</td><td style={tdStyle}>{row.operator}</td></>}
                    </tr>
                  );
                })}
              </tbody>
        ...
        {activeMenu === '1st Count' && isMobile && (
          <div style={formWrapper}>
                  ...
                  locInfo.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} style={{...infoLine, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        <div><b>{item.artikel}</b><br/><span style={{fontSize:'0.6rem'}}>{getDesc(item)}</span><br/>Snap: {item.qty_snap}</div>
            ...
            }} onKeyDown={e => { if(e.key === 'Enter') { if (inputQtyRef.current) inputQtyRef.current.focus(); } }} />
              ...
              {selectedLoc2nd && (
                <div style={formWrapper}>
                  ...
                  <input ref={inputLocRef} placeholder="Validasi Lokasi" value={mobLoc} style={mInput} onChange={e => {
                    const v = e.target.value.toUpperCase(); setMobLoc(v);
                    if(v === selectedLoc2nd.location_id.toUpperCase()) { if (inputArtRef.current) inputArtRef.current.focus(); }
                  }} />
                  <input ref={inputArtRef} placeholder="Validasi Artikel" value={mobArt} style={mInput} onChange={e => {
                    const v = e.target.value.toUpperCase(); setMobArt(v);
                    if(v === selectedLoc2nd.artikel.toUpperCase()) { if (inputQtyRef.current) inputQtyRef.current.focus(); }
                  }} />
                  <input ref={inputQtyRef} type="number" style={qtyInput} value={mobQty} onChange={e => setMobQty(e.target.value)} onKeyDown={e => { if(e.key === 'Enter') handleSaveInput(); }} />
...
/* ================= STYLES ================= */
const tabItem = (active) => ({ padding: '10px 15px', cursor: 'pointer', fontSize: '0.65rem', fontWeight: 800, color: active ? '#000' : '#ccc', borderBottom: active ? '2px solid #000' : 'none', display: 'flex', alignItems: 'center', gap: 5 });
